...
PLAYERCTL_PLAYER_OPT="--player spotify"
# SPT_FORMAT="%a - %t"
# SPT_STATUS_FORMAT="%s"
...
PLAYERCTL_STATUS=$(playerctl $PLAYERCTL_PLAYER_OPT status 2>/dev/null)
EXIT_CODE=$?
...
if [ "$1" == "--status" ]; then
	...
	if [ "$STATUS" = "Playing" ]; then
		update_hooks "$PARENT_BAR_PID" 1
		playerctl $PLAYERCTL_PLAYER_OPT metadata --format "$PLAYERCTL_FORMAT"
